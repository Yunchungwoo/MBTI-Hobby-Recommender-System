...
const Findtoidhome = () => {
  ...
  // 비밀번호 찾기 버튼 활성화 시에만 비밀번호 페이지로 링크 이동
  const handleSubmit = (e) => {
    e.preventDefault();
    if (selectedButton === "PASS") {
      navigate('/Find/Password'); // '/Find/Password'로 이동
    }
  };
      ...
      <div className="FindtoID-2">
        <div className="FindtoID-groupbox">
          <div className="FindtoID-selected">
            <div className="FindtoID-container" onSubmit={handleSubmit}>
              {/* ID 찾기 버튼 */}
              <img
          ...
          {/* div 클릭 시 handleSubmit 함수 호출 */}
          <div
            className="authentication-btn-email"
            onClick={handleSubmit}
            style={{ cursor: "pointer" }}
          />
          <div
            className="authentication-btn-email-text"
            onClick={handleSubmit}
            style={{ cursor: "pointer" }}
          >
            인증하기
